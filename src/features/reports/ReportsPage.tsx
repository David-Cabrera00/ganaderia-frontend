...
import { AlertAnalysisService, DashboardService } from "@/api/services";
...
export function ReportsPage() {
  ...
  const [aiSummary, setAiSummary] = useState<AlertAiSummaryResponse | null>(
    null,
  );
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [
        ...
        AlertAnalysisService.getAiSummary(),
      ]);
      ...
      if (alertsResult.status === "fulfilled") {
        setCriticalAlerts(alertsResult.value);
      }

      if (locationsResult.status === "fulfilled") {
        setRecentLocations(locationsResult.value);
      }

      if (cowsOutsideResult.status === "fulfilled") {
        setCowsOutside(cowsOutsideResult.value);
      }
      ...
      if (aiSummaryResult.status === "fulfilled") {
        ...
        setAiSummary(null);
        setAiError(AppError.from(aiSummaryResult.reason).serverMessage);
      }
      ...
      if (
        ...
        cowsOutsideResult.status === "rejected"
      ) {
  ...
  const loadAiSummary = useCallback(async () => {
    ...
    try {
      const data = await AlertAnalysisService.getAiSummary();
      setAiSummary(data);
    } catch (err) {
      setAiSummary(null);
      setAiError(AppError.from(err).serverMessage);
    } finally {
      setAiLoading(false);
    }
  }, []);
  ...
  const coveragePercentage = useMemo(() => {
    ...
    return Math.max(
      0,
      Math.round(
        ((summary.totalCows - summary.cowsOutsideGeofence) /
          summary.totalCows) *
          100,
      ),
    );
  ...
  const executiveActions = useMemo(() => {
    ...
    if (summary.pendingAlerts > 0) {
      actions.push(
        "Atender primero las alertas críticas y pendientes del turno.",
      );
    }

    if (summary.offlineCollars > 0) {
      actions.push(
        "Programar revisión técnica de collares sin señal o en mantenimiento.",
      );
    }

    if (summary.cowsOutsideGeofence > 0) {
      actions.push(
        "Verificar animales fuera del perímetro y validar el contexto del evento.",
      );
    }
  ...
  const aiSourceLabel = aiSummary?.fallbackUsed
    ? "Fallback heurístico"
    : aiSummary?.source === "AI"
      ? "IA Gemini"
      : "Análisis local";
  ...
  const displayedRecommendations =
    aiRecommendations.length > 0 ? aiRecommendations : executiveActions;
      ...
      <PageHeader
        ...
        actions={
          <Button
            ...
            onClick={() =>
              window.alert(
                "La exportación de reportes estará disponible próximamente.",
              )
            }
          >
            <Download size={14} />
      ...
      <PageContainer>
        ...
        <section className="card report-panel" style={{ marginTop: 18 }}>
          <div className="card-header report-panel-header">
            ...
            >
              <Sparkles size={14} />
              {aiLoading ? "Analizando..." : aiSourceLabel}
            </button>
          </div>
          ...
          <div className="card-body">
            <div className="reports-ai-summary">
              <div>
                <span className="reports-ai-summary-label">
                  Resumen generado
                </span>
                ...
                <p>
                  {aiSummary
                    ? aiSummary.summary
                    : aiError
                      ? "No se pudo cargar el análisis de IA. Se mantiene el análisis operativo local como respaldo."
                      : "El sistema está listo para generar análisis inteligente de alertas operativas."}
                </p>
              </div>

              <div className="reports-ai-summary-meta">
                <span>Nivel de riesgo</span>
                <strong>{aiRiskLabel}</strong>
              </div>
            </div>
                  ...
                  >
                    <div className="report-insight-top">
                      <div className="report-insight-heading">
                        <span
                          className={`report-insight-dot ${tone.dotClass}`}
                        />
                    ...
                    <p className="report-insight-description">
                      {item.description}
                    </p>
              ...
              <section className="report-subpanel">
                ...
                <div className="report-summary-grid">
                  <div className="report-summary-metric">
                    ...
                    <strong>
                      {formatDateTime(summary?.latestLocationTimestamp ?? null)}
                    </strong>
                  </div>
            ...
            <div className="report-detail-tabs">
              {[
                ...
                },
                {
                  key: "cows",
                  label: "Vacas fuera",
                  value: cowsOutside.length,
                },
                {
                  key: "locations",
                  label: "Trazabilidad",
                  value: recentLocations.length,
                },
              ].map((tab) => (
                <button
                  key={tab.key}
                ...
                <Table
                  columns={[
                    ...
                    },
                    {
                      key: "status",
                      header: "Estado",
                      render: (item: CowResponse) =>
                        COW_STATUS_LABELS[item.status],
                    },
                  ]}
                  data={cowsOutside}
